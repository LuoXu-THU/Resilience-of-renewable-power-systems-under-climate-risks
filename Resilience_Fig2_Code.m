load HurricaneEvents_Outage_fig2.mat
load Power_Rec_fig2.mat
...
for i = 1:length(D)
	...
	if input_index(i)
		if length(find(state_list == floor(Power_Rec_index1(i)/1000)))>0

			geoshow(D(i),'facecolor',test_color(color_idx(i),:),'edgecolor',[0.3,0.3,0.3],'LineWidth',0.3);
			hold on
		end
    end
    %end
